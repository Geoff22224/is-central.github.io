...
# Introduction
This guide provides an list of collectibles with a more detailed description of their relevant interactions. This page will basically always be a continuous work in progress, if you have any suggestions for improvements or additions or have noticed any mistakes, please contact me on discord at @kaltheresa or mention it in the IS Central discord. This guide was most recently updated on March 4th, 2026, for the pre-DLC1 post-IS6 release update. 

Across all IS's, collectibles are classified in different pools, these easily identifiable through their ingots price in shops (8, 12, 16). While these pools generally define the rarity of collectibles, it has been observed that within each tier some collectibles are noticeably more common than others but the respective weights are unknown.
...
### Internal modifiers
Internal modifiers are stat modifiers that applied after only after an operator is deployed (meaning they will not be reflected in an operator's stats when you select them from the deployment bar,) and are checked or triggered in a dynamic manner that depends on game statee. Examples of these checks include life point-related conditions (e.g. King's Crown), operator positioning or isolation conditions (e.g. Rusted Blade - Lone Force, Rock Horn), operator deployment time conditions (e.g. Hand of Protraction), and skill activation related conditions (e.g. Wrath of Siracusans, Black Tulip.) Internal modifiers are additive with modifiers from operator skills and talents, and are multiplicative with external modifiers (which include most collectible +atk% and +hp% effects). 
...
IS2: *Immediately gain +2 Objective HP and +1 Hope.*
IS3/4/5/6: *Max Life Point +1, Hope +1.*
...
IS2: *Bring a Mr. Boom along to each battle.* 
IS3/4/5/6: *Bring 2 Mr. Booms along to each battle.* 

The stun lasts for 12 seconds. 
...
IS2: *Bring a Snowsant's Safe Escape Crane along to each battle.* 
IS3/4/5/6": *Bring 2 Snowsant's Safe Escape Cranes along to each battle.* 
...
The healing effect triggers 1 second after deployment and is a heal for 1000 HP. 

It can heal the Last Knight.
...
IS2/3/4/5: *Defender Operators restore 2 SP after being attacked.*

IS6: *Defenders restore 2 SP each time they are attacked and their skills have -20% SP cost.*
...
IS2/3/4/5: *Enemies within the attack range of Supporter Operators have -20% DEF and -20% RES.*
IS6: *Enemies within a Supporter's attack range have -30% DEF and RES.*
...
In IS6: 
The -DEF/-RES effects of this collectible do not stack. 
...
IS3/4/5: *Allied units within the attack range of Supporter Operators gain +20% ATK.* 
IS6: *Friendly units within a Supporter's attack range have +30% ATK.*

This effect stacks in every IS theme that it appears in (including IS6, which is contradictory with how Stalwart Aid Counter-Arts works in IS6). If a unit is in the range of multiple supporters, they will benefit from multiple instances of this effect.

This effect is an internal/conditional buff.
...
IS4/5: *Treasure Chests on the battlefield will drop more Originium Ingots.*

IS6: *Gourds award more Originium Ingots when destroyed.* 
...
In IS6: 
This grants +4 ingots from Magic Gourds (analogous to standard treasure chests), +8 ingots from Iron Gourds (analogous to spiked chests), and +10 ingots from Strange Gourds (analogous to Vinecreep chests/Idle Exquisicarts). 
...
IS4/5: *Gain 1 Hope whenever you buy an item at the Rogue Trader or make an exchange at Lost and Found.*

IS6: *When an adventure is completed, 60% of the player's Originium Ingots at the time will be invested.*

In IS4/5: This grants you hope on the free "purchases" you can make while robbing Cannot's shop.
...
You can only obtain this collectible if you made it to at least F4 in the previous run and then died. You cannot get the Pure-White Petals and Wordless Deed thoughts (the ones required for ending 2) from Masterless Memories, but you can obtain Despair, Salvation and Stasis. If you obtain the ED5 thoughts this way, the encounter "All or Nothing" will still only appear after Finale Cadence is dead. If you want to achieve a run with multiple eggs/ED5 collectibles, you need to find multiple copies of the "All or Nothing" encounter, and can never get multiple instances of each egg (instead, the best you can do is T1 + T2 + T3 eggs).
...
If an operator was scouted away before this collectible was obtained, they will still bring back ingots. 

This works with operators scouted away by Witchcraft portals and Rhoders' Gate.
...
If the operator that was deployed last is taken by a Witchcraft portal, Rhoder's Gate will not take any operator. To use the effects of both, the second operator has to be deployed after the first operator is taken away by the portal.
# Combat Collectibles
## Max HP Collectibles
In IS2/3/4/5, the +max hp% effect is an external/unconditional modifier. In IS6, the +max hp% to all units effect is external/unconditional, while the +max hp% to Candleguest Operators effect is an internal/conditional modifier. 

- **Noxious Hemostatic Agent**
*IS2/IS3/IS4/IS5/IS6*

![collectible](https://i.imgur.com/B8ndmjI.png)

IS2/3/4/5: *All friendly units have +20% HP*

IS6: *Friendly units have +20% maximum HP, which is further increased by 10% for Candleguest Operators.*

- **First Aid Kit**
*IS2/IS3/IS4/IS5*

![collectible](https://i.imgur.com/OURy2tm.png)
...
![collectible](http://i.imgur.com/80Yetyj.png)

IS2/3/4/5: *All friendly units have +50% HP*
IS6:  *Friendly units have +50% maximum HP, which is further increased by 30% for Candleguest Operators.*
...
In IS5 at D18, the fractured -20% SP regen rate is applied multiplicatively to the result of this effect.

In IS6, if you have the Unflinching Masterplan collectible from ED3, the -50% initial SP effect is applied after every other SP calculation. This means you will never be able to deploy operators with more than 50% of the maximum SP. If you have Exusiai2 on S2 (which has an initial SP of 25/30), and +12 initial SP and Unflinching Masterplan, she will have (25+12)=37, which is larger than 30 so it is brought down to 30, then reduced by 50%, to 15, meaning she would deploy with 15/30 SP. 
...
Since the King's Crown condition is checked for dynamically, it is an internal/conditional modifier. This means that the +ATK% provided by King's Crown is additive with talent/skill +ATK% and multiplicative with external modifiers.
...
While this is not considered a King of Sarkaz's relic, it practically is a requirement to make them viable when also using King's relics. In IS5, the last point of Max Life gained from command level is at command level 6. This means that if you take Visage before command level 6, leveling up will ruin its ability to allow both the King's and King of Sarkaz's sets to be simultaneously active.
...
In IS2: *Heavyshooter, Deadeye, and Besieger Operators' attacks will defeat all non-Boss enemy units at 30% or less HP.*  

In IS3: *Heavyshooter, Deadeye, and Besieger Operators' attacks will instantly defeat enemies below 20% HP*  
...
IS3/IS4: *Deals 3000 True Damage on all enemy units every time an enemy unit enters the Protection Objective.*

IS5: Deals 5000 True Damage on all enemy units every time an enemy unit enters the Protection Objective
...
IS4: *Whenever an enemy enters the Objective Point, immediately selects a random Operator waiting to be re-deployable, and reduces their Redeployment Time by half.*

IS5: *Whenever an enemy enters the Objective Point, the Redeployment Time of a random Operator waiting to be re-deployed is reduced to 0.*

## *A random operator gains...* Relics
Stats provided by "When the battle begins, a random operator in your squad gains [various stat buffs]" effects are external modifiers. They provide conditional/external stat modifiers, meaning they are additive with effects like Emperor's Favor or Bend Spears - Acuity, and are multiplicative with internal modifiers (from operator talents and skills).
- **Bladedance**
*IS2*
...
Survivor Contract gains a stack at the end of each battle, and grants the random operator a stat buff equal to (number of stacks+1)*20%. 
...
This collectible automatically gains 1 out of the 10 stacks when obtained and caps at 10 stacks, unlike 1+10 stacks as it suggests.
...
Trading away these collectibles will make their effects disappear and also lower the number of stacks you have on Judgement collectibles.
...
While the mystery collectibles are supposed to work only when inside a Bizarre Fragment, the +20% Command EXP works on every stage outside as well.
...
IS2: *All friendly units have +5 RES.*

IS6: *Friendly units have +5 RES and +10% Arts Dodge.*
...
- **'Whiteflower'** *IS2* + **Vivaparous Lily** *IS3* + **Coin Geists** *IS6*
...
![collectible](https://i.imgur.com/EiRn8xe.png)

*In each operation, Operators have their DP cost shuffled.* 
...
IS3: *Designates one deployable tile at the beginning of battle; friendly units deployed on that tile will recover 1 SP and 3% Max HP per second.*

IS6: *A random deployable Vagary Tile will be selected in each operation; the friendly unit there restores 1 SP and regenerates 3% HP every second.*

In IS3: This collectible can choose some tiles that are not actually deployable on, like the water tiles on Cistern. The text of this collectible incorrectly states that this tick applies every 2 seconds, but it applies every second. 

In IS6: This collectible can only select deployable tiles. 
...
Many operators with summons can manually trigger the effect of Glory Pack if you place their summon down, then retreat the operator, causing the summon to disappear and trigger Glory Pack. This also works if a summon naturally disappears at the end of a duration. This interaction works with summons like Skalter's Seaborn and Pozyomka's Typewriter. For some reason, this does not work with specifically Kal'tsit's Mon3tr summon but it does with the summons of most ordinary summoners like Deepcolor. Support items (like Snowsant's Escape Crane) self-destructing after use trigger Glory Pack, as does THRM-EX self-destructing.
...
With the exception of Liberator Guards (due to their special mechanics), this will turn 0 block units (like ambushers) into 1 block units. This is generally undesirable.
...
IS4: *[Machine] and [Arts Creations] enemies will be immediately Frozen with their first attack for 15 seconds.*

IS6: *Apparition enemies are Feared for 5 seconds after their first attack.*

In IS4: This will generally screw you over in Manmade Carnival more than it helps. Be careful about picking this collectible.

In IS6: This will generally screw you over more than it helps. Be careful about picking this collectible.
...
These collectibles are all capable of desyncing the Liberator cycle on 4th ending. If you are going for 4th ending, you should consider skipping these (with that being said, the effect is definitely manageable and you don't have to skip them if you know what you are doing).
...
IS4: *Ranged Operators Levitate all enemies within attack range for 2 seconds when a skill is manually activated.*

IS6: *Whenever a ranged Operator with manual skill activates it, all enemies within their attack range are Levitated for 5 seconds.*
...
This collectible does not properly register +range effects like Ansel's +range on his skill or the +range that Archetto gets on her IS module. Enemies in range granted from range extension effects will not get levitated by Celestial Dust. 
...
The description specifies operators, but this grants shields to non-operator units that can take damage too, like summons or devices such as (both allied/enemy) 'Gelato Stop' from Divine Desire.
...
- Lowest rarity > highest toil > first obtained
...
The IS5 5th ending collectible "Reality of the End" brings you to a special Bizarre Fragment that inevitably ends with the run's loss if you fail any stage. If you have both Arch Glyph and "Reality of the End," the one that you obtained first is the one that triggers first. This means that if you obtained Arch Glyph after already obtaining "Reality of the End," Arch Glyph will not save you by default. However, Reality of the End is implemented in the same way as Arch Glyph, meaning that if you lose while in a Bizarre Fragment, Reality of the End will save your run but will not bring you into that special Bizarre Fragment. This means that if you received Arch Glyph after Reality of the End, you can use this trick to disable Reality of the End so you can use Arch Glyph.
...
![collectible](https://i.imgur.com/9XCtcQ2.png)
...
Having this collectible extensively modifies the IS5 ED5 fight, "Fate's Finale":

It causes Quillon + Buldrokkas'tee (with nerfed stats and some phases/abilities removed), as well as 1-3 Padmasanas of Rebirth/Boats to spawn in map 1. (Do also note that killing Quillon/Buldrokkas'tee will accelerate Amiya's Premonition of Continuation skill where she summons Originium over the map.) 

It causes Theresis + Manfred (with nerfed stats and some phases/abilities removed), as well as the Londinium Artillery to spawn on map 2. (Do also note that killing Theresis/Manfred will accelerate Amiya's Premonition of Continuation skill where she summons Originium over the map.) 

On the 3rd map, once Amiya has been pushed into her second form, the SP cost of Amiya's Premonition of Continuation skill will be reduced to 120 SP (instead of the 280 SP it is by default). 


If you have Reality of the End and lose on the way to the ED5 fight, Reality of the End will "save" your run but bring you to a special Bizarre Fragment that inevitably ends with the run's loss. If you have both Arch Glyph and Reality of the End, the one that you obtained first is the one that triggers first. This means that if you obtained Arch Glyph after already obtaining Reality of the End, Arch Glyph will not save you. However, Reality of the End is implemented in the same way as Arch Glyph, meaning that if you lose while in a Bizarre Fragment, Reality of the End will save your run but will not bring you into that special Bizarre Fragment. This means that if you received Arch Glyph after Reality of the End, you can use this trick to disable Reality of the End so you can use Arch Glyph.
